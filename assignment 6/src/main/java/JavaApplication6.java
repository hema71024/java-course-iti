...
public class JavaApplication6 {
    public static void main(String[] args) {
        try {
            ...
            DataFrame<Object> df1 = df.retain("name", "pclass").dropna().unique("name");
            DataFrame<Object> df2 = df.retain("name", "ticket").dropna().unique("name");
            DataFrame<Object> join= df1.join(df2,DataFrame.JoinType.INNER);
            System.out.println("DataFrame After Joining : ");
            System.out.println(join);
            DataFrame<Object> merge=df1.merge(df2, DataFrame.JoinType.OUTER);
